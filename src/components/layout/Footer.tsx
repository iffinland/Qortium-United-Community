...
import { BookOpen, MessageSquare, Heart, ExternalLink, Copy, Check, X, Coins, FolderKanban, ChevronUp, Calendar } from 'lucide-react';
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-1.5 text-[var(--color-text-muted)] transition hover:text-[var(--color-accent)]"
      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" onClick={() => setShowModal(false)}>
          <div className="w-full max-w-sm rounded-xl bg-[var(--color-surface-card)] p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between">
              ...
              <button onClick={() => setShowModal(false)} className="rounded-md p-1 text-[var(--color-text-muted)] hover:bg-slate-800">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="mb-4 flex items-center gap-2 rounded-lg border border-[var(--color-border-subtle)] bg-[var(--color-surface-muted)] p-3">
            ...
            <div className="relative">
              <button
                ...
                className={`flex w-full items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold text-white transition active:scale-95 ${
                  copied ? 'bg-emerald-600 shadow-lg shadow-emerald-500/30' : 'bg-[var(--color-accent)] hover:bg-[var(--color-accent-hover)]'
                }`}
              >
...
const Footer = () => {
  ...
  return (
    <footer className="border-t border-[var(--color-border)] bg-[var(--color-footer-drawer)]">
      <div className="flex justify-center">
        <button
          type="button"
          onClick={() => setIsOpen((open) => !open)}
          aria-expanded={isOpen}
          aria-controls="quc-footer-panel"
          data-state={isOpen ? 'open' : 'closed'}
          className="quc-footer-handle"
        >
          <ChevronUp
            className={`h-4 w-4 transition-transform ${isOpen ? 'rotate-180' : ''}`}
            aria-hidden="true"
          />
          <span>{isOpen ? 'Close footer panel' : 'Open footer panel'}</span>
        </button>
      </div>

      <div
        id="quc-footer-panel"
        data-testid="quc-footer-panel"
        aria-hidden={!isOpen}
        inert={!isOpen}
        className={`quc-footer-panel ${isOpen ? 'is-open' : ''}`}
      >
          ...
          <div className="grid gap-8 sm:grid-cols-3">
            ...
            <div>
              ...
              <nav className="grid grid-cols-2 gap-1 text-xs">
                <Link to="/wiki" className="flex items-center gap-1.5 py-1 text-[var(--color-text-muted)] transition hover:text-[var(--color-accent)]">
                  <BookOpen className="h-3 w-3" /> Wiki
                </Link>
                <Link to="/forum" className="flex items-center gap-1.5 py-1 text-[var(--color-text-muted)] transition hover:text-[var(--color-accent)]">
                  <MessageSquare className="h-3 w-3" /> Forum
                </Link>
                <Link to="/support" className="flex items-center gap-1.5 py-1 text-[var(--color-text-muted)] transition hover:text-[var(--color-accent)]">
                  <Heart className="h-3 w-3" /> Support
                </Link>
                <Link to="/donations" className="flex items-center gap-1.5 py-1 text-[var(--color-text-muted)] transition hover:text-[var(--color-accent)]">
                  <Coins className="h-3 w-3" /> Donate
                </Link>
                <Link to="/projects" className="flex items-center gap-1.5 py-1 text-[var(--color-text-muted)] transition hover:text-[var(--color-accent)]">
                  <FolderKanban className="h-3 w-3" /> Projects
                </Link>
                <Link to="/events" className="flex items-center gap-1.5 py-1 text-[var(--color-text-muted)] transition hover:text-[var(--color-accent)]">
